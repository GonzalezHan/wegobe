...
const GasIntro = () => {
  ...
  useEffect(() => {
      ...
      intervalId = window.setInterval(() => {
        setTypedUnits((prev) => {
          if (prev >= totalUnits) {
            ...
            // Only restart if we've played less than 2 times
            if (playCountRef.current < 2) {
              loopTimeout = window.setTimeout(() => {
                runAnimation();
              }, 1500); // 1.5s delay between plays
            }
    ...
    const initialTimeoutId = window.setTimeout(runAnimation, 280);
    ...
    return () => {
      ...
      // Reset play count when out of view/unmount to allow replay when user scrolls back
          ...
          <div className="grid w-full items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)]">
            <div className="max-w-[540px] py-10">
                ...
                <p className="mt-7 max-w-[520px] text-[17px] leading-[1.65] text-gray-600">
                  서버나 복잡한 설치 없이 우리가 매일 사용하는 구글 워크스페이스(드라이브, 시트, 슬라이드, 독스)를 프로그래밍하고
